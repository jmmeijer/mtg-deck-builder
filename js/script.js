...
const DEFAULT_SEED_URL = 'data/seed-cards.json';
...
let seed = [];
let state = { cards: [], ui: { stacked: { cut: true } } };
let dragged = null;
...
async function loadSeedData() {
  try {
    const response = await fetch(DEFAULT_SEED_URL, { cache: 'no-cache' });
    if (!response.ok) throw new Error(`Could not load ${DEFAULT_SEED_URL}`);

    const data = await response.json();
    seed = Array.isArray(data) ? data : Array.isArray(data?.cards) ? data.cards : [];
  } catch (error) {
    console.warn('Could not load default seed data. Starting with an empty board.', error);
    seed = [];
  }
}
...
function makeCard(entry) {
  const card = Array.isArray(entry)
    ? { name: entry[0], section: entry[1], status: entry[2], role: entry[3] }
    : entry || {};
  ...
  return {
    ...
    status: card.status || 'New candidate',
    role: card.role || '',
    scryfall: null,
    error: null
  };
...
function bindEvents() {
  ...
  if (exportButton) exportButton.onclick = exportJsonDownload;
